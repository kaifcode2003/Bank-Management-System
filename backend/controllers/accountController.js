...
exports.getAccount = (req, res) => {
    ...
    db.get(query, [userId], (err, accountData) => {
        if (err || !accountData) return res.status(404).json({ message: 'Account not found.' });

        db.all('SELECT * FROM transactions WHERE account_id = ? ORDER BY timestamp DESC', [accountData.account_id], (txErr, transactions) => {
            if (txErr) return res.status(500).json({ message: 'Error fetching transactions.' });
